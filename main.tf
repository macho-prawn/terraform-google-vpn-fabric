resource "google_compute_router" "router" {
  for_each = local.vpn_details_map
  name     = "cr-${each.value.env_name}-${each.value.gw_index[0]}-${each.value.region_abbr}-${each.value.gw_name}"
...
resource "google_compute_ha_vpn_gateway" "gateway" {
  ...
  name    = "gw-${each.value.env_name}-${each.value.gw_index[0]}-${each.value.region_abbr}-${each.value.gw_name}"
  project = each.value.project
  region  = each.value.region
  network = each.value.vpc
}
...
resource "google_compute_vpn_tunnel" "peer1_tunnel_0" {
  ...
  ike_version              = 2
  router                   = google_compute_router.router[each.value.peer1_gws.gateway_key].name
  vpn_gateway              = google_compute_ha_vpn_gateway.gateway[each.value.peer1_gws.gateway_key].id
  vpn_gateway_interface    = each.value.peer1_tunnels.interface
  peer_gcp_gateway         = google_compute_ha_vpn_gateway.gateway[each.value.peer2_gws.gateway_key].id
}
...
resource "google_compute_vpn_tunnel" "peer2_tunnel_0" {
  ...
  ike_version              = 2
  router                   = google_compute_router.router[each.value.peer2_gws.gateway_key].name
  vpn_gateway              = google_compute_ha_vpn_gateway.gateway[each.value.peer2_gws.gateway_key].id
  vpn_gateway_interface    = each.value.peer2_tunnels.interface
  peer_gcp_gateway         = google_compute_ha_vpn_gateway.gateway[each.value.peer1_gws.gateway_key].id
}
