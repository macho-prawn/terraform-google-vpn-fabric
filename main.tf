...
resource "google_compute_vpn_tunnel" "peer1_tunnel_0" {
  ...
  region                   = each.value.peer1_gws.region
  ike_version              = 2
  router                   = google_compute_router.peer1_router["${each.value.peer1_gws.env_key}-${each.value.peer2_gws.env_key}"].name
  vpn_gateway              = google_compute_ha_vpn_gateway.peer1_gateway["${each.value.peer1_gws.env_key}-${each.value.peer2_gws.env_key}"].id
  vpn_gateway_interface    = each.value.peer1_tunnels.interface
  peer_gcp_gateway         = google_compute_ha_vpn_gateway.peer2_gateway["${each.value.peer1_gws.env_key}-${each.value.peer2_gws.env_key}"].id
}
...
resource "google_compute_vpn_tunnel" "peer2_tunnel_0" {
  ...
  region                   = each.value.peer2_gws.region
  ike_version              = 2
  router                   = google_compute_router.peer2_router["${each.value.peer1_gws.env_key}-${each.value.peer2_gws.env_key}"].name
  vpn_gateway              = google_compute_ha_vpn_gateway.peer2_gateway["${each.value.peer1_gws.env_key}-${each.value.peer2_gws.env_key}"].id
  vpn_gateway_interface    = each.value.peer2_tunnels.interface
  peer_gcp_gateway         = google_compute_ha_vpn_gateway.peer1_gateway["${each.value.peer1_gws.env_key}-${each.value.peer2_gws.env_key}"].id
}
...
resource "google_compute_router_interface" "peer1_cloud_router_interface" {
  ...
  name       = startswith("${each.key}", "0") ? "if-bgp-${google_compute_vpn_tunnel.peer1_tunnel_0[each.key].name}" : "if-bgp-${google_compute_vpn_tunnel.peer1_tunnel_1[each.key].name}"
  project    = each.value.peer1_gws.project
  router     = google_compute_router.peer1_router["${each.value.peer1_gws.env_key}-${each.value.peer2_gws.env_key}"].name
  region     = each.value.peer1_gws.region
  vpn_tunnel = startswith("${each.key}", "0") ? google_compute_vpn_tunnel.peer1_tunnel_0[each.key].name : google_compute_vpn_tunnel.peer1_tunnel_1[each.key].name
}
...
resource "google_compute_router_peer" "peer1_router_peer" {
  ...
  name           = startswith("${each.key}", "0") ? "bgp-${google_compute_vpn_tunnel.peer1_tunnel_0[each.key].name}" : "bgp-${google_compute_vpn_tunnel.peer1_tunnel_1[each.key].name}"
  project        = each.value.peer1_gws.project
  router         = google_compute_router.peer1_router["${each.value.peer1_gws.env_key}-${each.value.peer2_gws.env_key}"].name
  interface      = startswith("${each.key}", "0") ? "if-bgp-${google_compute_vpn_tunnel.peer1_tunnel_0[each.key].name}" : "if-bgp-${google_compute_vpn_tunnel.peer1_tunnel_1[each.key].name}"
...
resource "google_compute_router_interface" "peer2_cloud_router_interface" {
  ...
  name       = startswith("${each.key}", "0") ? "if-bgp-${google_compute_vpn_tunnel.peer2_tunnel_0[each.key].name}" : "if-bgp-${google_compute_vpn_tunnel.peer2_tunnel_1[each.key].name}"
  project    = each.value.peer2_gws.project
  router     = google_compute_router.peer2_router["${each.value.peer1_gws.env_key}-${each.value.peer2_gws.env_key}"].name
  region     = each.value.peer2_gws.region
  ip_range   = "${google_compute_router_peer.peer1_router_peer[each.key].peer_ip_address}/30"
  vpn_tunnel = startswith("${each.key}", "0") ? google_compute_vpn_tunnel.peer2_tunnel_0[each.key].name : google_compute_vpn_tunnel.peer2_tunnel_1[each.key].name
}
...
resource "google_compute_router_peer" "peer2_router_peer" {
  ...
  name            = startswith("${each.key}", "0") ? "bgp-${google_compute_vpn_tunnel.peer2_tunnel_0[each.key].name}" : "bgp-${google_compute_vpn_tunnel.peer2_tunnel_1[each.key].name}"
  project         = each.value.peer2_gws.project
  router          = google_compute_router.peer2_router["${each.value.peer1_gws.env_key}-${each.value.peer2_gws.env_key}"].name
  interface       = startswith("${each.key}", "0") ? "if-bgp-${google_compute_vpn_tunnel.peer2_tunnel_0[each.key].name}" : "if-bgp-${google_compute_vpn_tunnel.peer2_tunnel_1[each.key].name}"
