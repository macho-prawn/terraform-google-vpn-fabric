...
control "gcp-vpn-destination-1.0" do
  ...
  dst_projects.each do |dst_project_id, dst_project|
    dst_region = dst_project.fetch("region")
    dst_router_names = dst_project.fetch("router_names")
    dst_gateway_names = dst_project.fetch("gateway_names")
    dst_tunnel_names = dst_project.fetch("tunnel_names")
